...
class Post:
    ...
    # here we save posts of a user
    def savee(self, user_name):
        file_object = open("posts_of_{}.txt".format(user_name), "a+") 
        file_object.seek(0)
        data = file_object.read(100)
        file_object.write("\n")
        file_object.write(self.text)
        file_object.write("\n")
        file_object.write(str(self.Date))
        file_object.write("\n")
        for i in range(self.num_of_comments):
            file_object.write(self.comments[i])
            file_object.write("\n")
        file_object.close()
